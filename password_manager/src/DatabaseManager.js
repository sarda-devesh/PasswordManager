...
let table_name = "[password_table_name]";
...
function get_password(user_name, label, result_callback) { 
    get_information(user_name, label, (result) => {
        ...
        if(result.length != 0) {
            let data = result[0];
            result_message = `Login: ${data.login} \nPassword: ${data.password}`;
        } else {
    ...
    get_information(user_name, label, (result) => {
        ...
        if(result.length != 0) {
            ...
            if(updated_login != data.login || updated_password != data.password) {
                sql_command = ` UPDATE ${table_name} SET login = '${updated_login}', password = '${updated_password}'
...
module.exports = { get_password, update_password, remove_password, add_password};